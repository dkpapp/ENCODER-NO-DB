...
async def en_co_de(dl, message):
  ...
  if codec[0] == "libx265":
    if audio_codec[0] == "libfdk_aac":
      ...
      cmd = f'''ffmpeg -hide_banner -loglevel quiet -progress """{progress}""" -i """{dl}""" -filter_complex "drawtext=fontfile=njnaruto(1).ttf:fontsize={watermark_size[0]}:fontcolor=white:bordercolor=black@0.50:x=10:y=10:box=1:boxcolor=black@0.5:boxborderw=6:text='Animes Encoded':enable='between(t,0,15)':alpha='if(lt(t,14)\,1\,if(lt(t\,15)\,(1-(t-14))/1\,0))'" -metadata:s:a:0 title="[Telegram @Animes_Encoded]" -metadata:s:a:1 title="[Telegram @Animes_Encoded]" -metadata:s:s:0 title="[Telegram @Animes_Encoded]" -metadata:s:s:1 title="[Telegram @Animes_Encoded]" -map 0:v? -map 0:a? -map 0:s? -c:v libx265 -x265-params no-info=1 -crf {crf[0]} -s {qualityy[0]} -b:v 420k -preset {preset[0]} -threads 3 -pix_fmt yuv420p -c:a libopus -profile:a aac_he -ac 2 -b:a 36k -c:s copy """{ul}""" -y'''
  else:
    if audio_codec[0] == "libopus":
      cmd = f'''ffmpeg -hide_banner -loglevel quiet -progress """{progress}""" -i """{dl}""" -map 0:a? -map 0:s? -map 0:v? -c:v libx264 -crf {crf[0]} -pix_fmt yuv420p -s {qualityy[0]} -preset {preset[0]} -c:a libopus -profile:a aac_he -ac 2 -b:a 36k -c:s copy """{ul}""" -y'''
